...
export default function reactivate(stage, item) {
    ...
    */
    ...
    /* II Recovering Images */
    stage.find('Image').destroy();
    var ZImages = []; // This array will load all images from item.
    for (let i = 0; i < item.images.length; i++) {
        ZImages[i] = new Image();
        ZImages[i].onload = function() {
            var pic = new Konva.Image({
                ...
                scaleX: item.images[i].scaleX(),
                scaleY: item.images[i].scaleY(),
                name: item.images[i].attrs.name,
                legend: item.images[i].attrs.legend,
                id: item.images[i].id(),
                draggable: true
            });
            ...
            layer.batchDraw();
            pic.setAttr("source", item.images[i].attrs.source)
            pic.setAttr("belongTo", item.images[i].attrs.belongTo)
        };
        ZImages[i].src = item.images[i].attrs.source;
    }
    ...
    var bubblePosiX, bubblePosiY
    if (typeof item.hintBubblePosi == 'undefined') {
        bubblePosiX = 200;
        bubblePosiY = 550;
    } else {
        bubblePosiX = item.hintBubblePosi[0];
        bubblePosiY = item.hintBubblePosi[1];
    }
    ...
    imageObj1.onload = function() {
        ...
        //HINT Keep position
        var keepHint = stage.findOne("#hint");
        console.log(keepHint)
        keepHint.on("dragend", function() {
                item.hintBubblePosi = [this.x(), this.y()]
                console.log(item)
            })
            // add the shape to the layer
    ...
    imageObj1.src = blightSRC;


    var hintContent = stage.find("#hintContent");
    //Activation of the Editor for Hint content if is text!
    ...
    for (let i = 0; i < item.choices.length; i++) {
        var GroupChoice = item.choices[i].scope;
        if (item.choices[i].type == "checkbox") {
            cloneCk(stage, "checkbox", GroupChoice, layer)
        } else if (item.choices[i].type == "radio") {
    ...
    for (let i = 0; i < allFeedBack.length; i++) {
        if (allFeedBack[i].getClassName() == "Text") {
            allFeedBack[i].attrs.legend = allFeedBack[i].text();
            ...
            allNGroups[i].on("click", function() {
                ...
                for (let y = 0; y < allFeedBack.length; y++) {
                    if (allFeedBack[y].attrs.belongTo == allNGroups[i].id() && allNGroups[i].findOne(".tick").attrs.visible && !item.eliminationMode) {
                        ...
                        if (allFeedBack[y].getClassName() == "Text") { allFeedBack[y].attrs.legend = allFeedBack[y].text(); }
                        layer.draw()
                    } else {
    ...
    var allShapes = stage.find(".freeShape");
    //Build Line array
    var GrouplineArray = [];
    for (let i = 0; i < allShapes.length; i++) {
        if (allShapes[i].attrs.type == "line") {
            GrouplineArray.push(allShapes[i])
        }
    }
    //Use Line Array to generate all Lines
    for (let i = 0; i < GrouplineArray.length; i++) {
        var redLine = GrouplineArray[i].findOne('.redLine')
        var Handlers = GrouplineArray[i].find(".lineHandler")
        var C1Handler = Handlers[0];
        var C2Handler = Handlers[1];
